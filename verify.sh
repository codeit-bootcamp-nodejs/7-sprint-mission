...
PRODUCTION_URL="https://sprint3-7qu0.onrender.com"

# If PRODUCTION_URL is set and reachable, use it. Otherwise use localhost.
# For this script, since user asked to verify specific URL, we set BASE_URL.
...
curl -s -o /dev/null -w "%{http_code}" -X GET $BASE_URL/api/products
echo -e "\n(Expected: 200)"

# 2. CORS Test
echo "1-2. CORS Test: OPTIONS /api/products"
curl -I -X OPTIONS $BASE_URL/api/products \
...
ARTICLE_RESP=$(curl -s -X POST $BASE_URL/api/articles \
...
if [ -n "$ARTICLE_ID" ]; then
  ...
  curl -s -X GET "$BASE_URL/api/articles?search=Searchable"
  echo -e "\n"

  # Search Content
  echo "2-3. Search Article (Content)"
  curl -s -X GET "$BASE_URL/api/articles?search=Content"
  echo -e "\n"

  # Add Comment to Article
  echo "2-4. Add Comment to Article $ARTICLE_ID"
  curl -s -X POST $BASE_URL/api/articles/$ARTICLE_ID/comments \
    -H "Content-Type: application/json" \
  ...
  # First page
  RESP_PAGE1=$(curl -s -X GET "$BASE_URL/api/articles/$ARTICLE_ID/comments?limit=1")
  ...
  if [ -n "$CURSOR" ]; then
    ...
    curl -s -X GET "$BASE_URL/api/articles/$ARTICLE_ID/comments?limit=1&cursorId=$ENCODED_CURSOR"
    echo -e "\n"
  else
...
if [ -n "$PRODUCT_ID" ]; then
  ...
  curl -s -X GET "$BASE_URL/api/products?search=Product"
  echo -e "\n"

  # Search Description
  echo "3-3. Search Product (Description)"
  curl -s -X GET "$BASE_URL/api/products?search=Description"
  echo -e "\n"

  # Add Comment to Product
  echo "3-4. Add Comment to Product $PRODUCT_ID"
  COMMENT_RESP=$(curl -s -X POST $BASE_URL/api/products/$PRODUCT_ID/comments \
    -H "Content-Type: application/json" \
  ...
  # Add another comment
  curl -s -X POST $BASE_URL/api/products/$PRODUCT_ID/comments \
    -H "Content-Type: application/json" \
    -d '{"content": "Product Comment 2"}' > /dev/null

  # List Product Comments (Basic)
  echo "3-5. List Product Comments (Basic limit=5)"
  curl -s -X GET "$BASE_URL/api/products/$PRODUCT_ID/comments?limit=5"
  echo -e "\n"

  # List Product Comments (Cursor)
  echo "3-6. List Product Comments (Cursor)"
  RESP_PROD_PAGE1=$(curl -s -X GET "$BASE_URL/api/products/$PRODUCT_ID/comments?limit=1")
  ...
  if [ -n "$PROD_CURSOR" ]; then
    ...
    echo "Fetching Page 2 with cursor: $ENCODED_CURSOR"
    curl -s -X GET "$BASE_URL/api/products/$PRODUCT_ID/comments?limit=1&cursorId=$ENCODED_CURSOR"
    echo -e "\n"
  fi
  ...
  curl -s -o /dev/null -w "%{http_code}" -X PATCH "$BASE_URL/api/comments/99999999" \
    -H "Content-Type: application/json" \
  ...
  curl -s -X POST $BASE_URL/api/upload/image \
    -F "image=@test.jpg"
  echo -e "\n"
  ...
  curl -s -o /dev/null -w "%{http_code}" -X POST $BASE_URL/api/upload/image \
    -F "image=@test.txt"
  ...
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X GET "$BASE_URL/api/products/$PRODUCT_ID/comments")
