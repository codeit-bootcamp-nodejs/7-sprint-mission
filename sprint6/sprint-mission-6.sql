...
SELECT pizza_id FROM order_details WHERE pizza_id IN (SELECT id FROM pizzas WHERE price > 20)
GROUP BY pizza_id;
...
SELECT O.date,COUNT(O.id) AS order_count,SUM(OD.quantity) AS total_quantity FROM orders O
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT P.*,SUM(OD.quantity) AS total_quantity  FROM pizzas P 
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
SELECT O.date, COUNT(DISTINCT O.id) AS total_orders, SUM(P.price * OD.quantity) AS total_amount FROM orders O 
...
SELECT P.size ,SUM(P.price * OD.quantity) AS total_amount FROM order_details OD
JOIN pizzas P ON p.id = OD.pizza_id
GROUP BY P.size
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT PT.name,SUM(P.price * OD.quantity) AS total_amount FROM order_details OD
JOIN pizzas P ON OD.pizza_id = P.id
