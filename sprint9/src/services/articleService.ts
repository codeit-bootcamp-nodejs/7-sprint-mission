...
export async function createComment(data: {
  ...
  if (!existingArticle) {
    ...
    if(data.articleId){
        const article = await articleRepository.getArticleWithLikes(data.articleId);
        ...
        if(articleWriterId !== commentWriterId){
           const createdNotification = await notificationService.createNotification({
                userId: articleWriterId as number,
        ...
        socketService.sendNotification(createdNotification);                     
        }
    }
...
export async function getCommentList(params: {
    ...
    const hasNextPage = commentsWithCursor.length > params.limit;
    const comments = hasNextPage ? commentsWithCursor.slice(0, params.limit) : commentsWithCursor;
