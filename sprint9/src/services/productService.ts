...
export async function updateProduct(id: number, data: Partial<Product>, user?: { id: number }){
    ...
    if (previousPrice !== updatedPrice) {
        ...
        // Create each notification so we get Notification objects (not BatchPayload)
        const createdNotifications = await Promise.all(
            notifications.map(notification => notificationsService.createNotification(notification))
        );
        // Send each created notification via socket
